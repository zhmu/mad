...
*ARCHIVE::readfile(char *fname,_ULONG *rsize) {
    ...
    /* is this stuff compressed? */
    if (entry.real_size) {
        /* yup. allocate a buffer for the uncompressed stuff */
        if((tmp=malloc(entry.real_size))==NULL) {
            /* error 2: out of memory */
            sprintf(tempstr,MAD_ERROR_2,entry.real_size);
            die(tempstr);
        }

        /* uncompress the buffer */
        real_size=entry.real_size;
        if((pos=uncompress((Bytef*)tmp,&real_size,(Bytef*)ptr,size))!=Z_OK) {
            /* this failed. (error 18: compression code error) */
            die(MAD_ERROR_18);
        }

        /* free the old buffer and use this new one with the correct size */
        free(ptr); size=real_size; ptr=tmp;
    }
...
*ARCHIVE::err2msg(_UCHAR err) {
    switch (err) {
     ...
     case ARCHIVE_NOTINARCHIVE: return ARCHIVE_ERROR_8;
      ...
      case ARCHIVE_COMPRESSERR: return ARCHIVE_ERROR_10;
    }
