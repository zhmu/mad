...
#define MAD_ERROR_705 "Error 705: 24 or 32 bit color mode required"
...
GFXDRIVER::init() {
  ...
  if ((depth != 32) && (depth != 24)) {
      /* no. complain (error 705: 24 or 32 bit color mode required) */
      die(MAD_ERROR_705);
  }
  ...
  ximage = XCreateImage (display, visual, depth, ZPixmap, 0, NULL,
                         GFX_DRIVER_HRES, GFX_DRIVER_VRES, 32, 0);
  if (ximage == NULL) {
...
void
GFXDRIVER::draw(void *data) {
  memcpy(ximage->data, data, (GFX_DRIVER_HRES*GFX_DRIVER_VRES*GFX_BPP));
