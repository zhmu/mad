...
GFXDRIVER::init() {
  ggi_mode mode = { 1,                                       /* number of frames */
                    { GFX_DRIVER_HRES, GFX_DRIVER_VRES },    /* resolution */
                    { GFX_DRIVER_HRES, GFX_DRIVER_VRES },    /* virtual */
                    { 0, 0 },                                /* size in mm */
                    GT_AUTO,                                 /* color depth */
...
void
GFXDRIVER::draw(void *data) {
     ggiPutBox (ggiVisual, 0, 0, GFX_DRIVER_HRES, GFX_DRIVER_VRES, data);
    /*for (i=0;i<GFX_DRIVER_VRES;i++) {
        for(j=0;j<GFX_DRIVER_HRES;j++) {
            col=ptr[GFX_DRIVER_HRES*i+j];
	    tempbuf[j]=palette_color[col];
	}
        ggiPutHLine (ggiVisual, 0, i, GFX_DRIVER_HRES, tempbuf);
    }*/
}
