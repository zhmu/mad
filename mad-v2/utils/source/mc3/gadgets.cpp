...
/*
 * gad_resolvetype(char* str)
 *
 * This will resolve string [str] to a variable type. It will return
 * OBJMAN_TYPE_UNUSED when the type could not be resolved, otherwise
 * OBJMAN_TYPE_xxx.
 *
 */
_UCHAR
gad_resolvetype(char* str) {
    /* is it a variable? */
    if(!strcmp(str,MC_DEFINE_VAR)) return OBJMAN_TYPE_VARIABLE;
    /* is it an animation? */
    if(!strcmp(str,MC_DEFINE_ANM)) return OBJMAN_TYPE_ANM;
    /* is it text? */
    if(!strcmp(str,MC_DEFINE_TEXT)) return OBJMAN_TYPE_TEXT;

    /* it's unknown. return OBJMAN_TYPE_UNUSED */
    return OBJMAN_TYPE_UNUSED;
}
