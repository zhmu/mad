...
#define MAD_GFXDRIVER_NAME "window.dll"
...
GFXDRIVER::init() {
	...
	if ((gfx_init = (GFXFUNC) GetProcAddress (gfxdrv, "gfx_init")) == NULL) {
		/* Damn, the driver doesn't contain the init function. die */
		die ("Graphics driver doesn't have an init() function");
	...
	if ((gfx_done = (GFXFUNC) GetProcAddress (gfxdrv, "gfx_done")) == NULL) {
		/* Damn, the driver doesn't contain the done function. die */
		die ("Graphics driver doesn't have a done() function");
	...
	if ((gfx_draw = (GFXDRAW) GetProcAddress (gfxdrv, "gfx_draw")) == NULL) {
	...
	if ((gfx_setpalette = (GFXDRAW) GetProcAddress (gfxdrv, "gfx_setpalette")) == NULL) {
	...
	if ((gfx_resolverror = (GFXRESOLVERROR) GetProcAddress (gfxdrv, "gfx_resolverror")) == NULL) {
        /* Damn, the driver doesn't contain the setpalette function. die (error 12: graphics driver doesn't contain the function) */
		sprintf(tempstr,MAD_ERROR_12,"resolverror()");
		die(tempstr);
	}
	if ((gfx_poll = (GFXPOLL) GetProcAddress (gfxdrv, "gfx_poll")) == NULL) {
        /* Damn, the driver doesn't contain the setpalette function. die (error 12: graphics driver doesn't contain the function) */
	...
	if (gfxdriver->get_caps() & GFX_DRIVER_CAPS_ALSOKBD) {
		/* Oh boy, this driver also does controls! Create the right pointers to procedures */
   	    if ((gfx_get_keyboard_status = (GFXGETKBDSTAT) GetProcAddress (gfxdrv, "gfx_get_keyboard_status")) == NULL) {
            /* Damn, the driver doesn't contain the setpalette function. die (error 12: graphics driver doesn't contain the function) */
		    sprintf(tempstr,MAD_ERROR_12,"get_keyboard_status");
		    die(tempstr);
	    }
   	    if ((gfx_setquithandler = (GFXDRAW) GetProcAddress (gfxdrv, "gfx_setquithandler")) == NULL) {
            /* Damn, the driver doesn't contain the setpalette function. die (error 12: graphics driver doesn't contain the function) */
	...
	if (gfxdriver->get_caps() & GFX_DRIVER_CAPS_ALSODEV) {
        /* Yeah. Well, let use them! */
   	    if ((gfx_get_device_status = (GFXGETDEVSTAT) GetProcAddress (gfxdrv, "gfx_get_device_status")) == NULL) {
            /* Damn, the driver doesn't contain the setpalette function. die (error 12: graphics driver doesn't contain the function) */
