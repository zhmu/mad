...
do_conversation(_UINT x,_UINT y,_UINT unselcol,_UINT selcol,_ULONG no) {
    ...
    save_mousesprno=objman->getsourceno(OBJMAN_MOUSE_OBJNO);
    mouse_enabled=objman->isenabled(OBJMAN_MOUSE_OBJNO);

    objman->setsourceno(OBJMAN_MOUSE_OBJNO,controls->sysmouse_sprno);
    objman->setenabled(OBJMAN_MOUSE_OBJNO,1);
    ...
    while(1) {
        ...
        if(controls->button1pressed()&&(i<nofstrings)) break;
    }
    ...
    mainscript->dialogno=old_dlg_no;

    /* restore the mouse sprite */
    objman->setsourceno(OBJMAN_MOUSE_OBJNO,save_mousesprno);
    objman->setenabled(OBJMAN_MOUSE_OBJNO,mouse_enabled);
