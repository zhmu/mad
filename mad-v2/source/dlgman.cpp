 ...
 */
void
CONTROL::setcolor(_UCHAR desired_color) {
    color=desired_color;
}
...
CONTROL::draw(char* dest,_SINT basex,_SINT basey) {
    ...
    switch(type) {
         case DLGMAN_CONTROL_TYPE_LABEL: /* We are a label! */
                                         fontman->textout(fontman->getsysfontno(),color,caption,basex+x,basey+y,dest);
                                         break;
        case DLGMAN_CONTROL_TYPE_BUTTON: /* We are a button! */
                                         ...
                                         /* draw the caption */
                                         fontman->textout(fontman->getsysfontno(),DLGMAN_DIALOG_TITLECOLOR,caption,basex+x+((width-caption_width)/2),y+basey+1,dest);
                                         break;
     case DLGMAN_CONTROL_TYPE_TEXTINPUT: /* We are a text input box! */
                                         ...
                                         /* draw the caption */
                                         fontman->textout(fontman->getsysfontno(),color,caption,basex+x,y+basey,dest);
                                         /* and draw the cursor (if we have the focus) */
                                         if(focus) {
                                             fontman->textout(fontman->getsysfontno(),color,DLGMAN_CONTROL_CURSOR,basex+x+fontman->getextwidth(fontman->getsysfontno(),caption),y+basey,dest);
                                         }
                                         break;
          case DLGMAN_CONTROL_TYPE_LIST: /* We are a list! */
                                         ...
                                         /* draw the 'up' text */
                                         fontman->textout(fontman->getsysfontno(),color,DLGMAN_LIST_UPBUTTON,x+basex+(width-fontman->getextwidth(fontman->getsysfontno(),DLGMAN_LIST_UPBUTTON))/2,y+basey,dest);
                                         ...
                                         for(j=0;j<i;j++) {
                                             ...
                                             fontman->textout(fontman->getsysfontno(),c,tempstr,x+basex,y+basey+((j+1)*fontman->getfontheight(fontman->getsysfontno())),dest);
                                         }
                                         ...
                                         /* draw the 'down' text */
                                         fontman->textout(fontman->getsysfontno(),color,DLGMAN_LIST_DNBUTTON,x+basex+(width-fontman->getextwidth(fontman->getsysfontno(),DLGMAN_LIST_DNBUTTON))/2,y+basey+((n+1)*fontman->getfontheight(fontman->getsysfontno()))+1,dest);
                                         break;
...
DIALOG::init(_UINT theflags,_SINT thex,_SINT they,_UINT theheight,_UINT thewidth) {
    ...
    strcpy(title,DLGMAN_DIALOG_DEFAULTITLE);
...
DIALOG::draw(char* dest) {
    ...
    if(backdrop==GFX_NOSPRITE) {
        ...
        if(titley) {
            ...
            fontman->textout(fontman->getsysfontno(),DLGMAN_DIALOG_TITLECOLOR,title,x+2,y+1,dest);
        }
        /* draw the border (|-) */
...
DLGMAN::drawdialog(_UINT no,_SINT device_x,_SINT device_y,char* displaydata) {
    ...
    if(!(dialog[no]->getflags()&DLGMAN_DIALOG_FLAG_NOMOUSE)) {
        ...
        if(objman->getsourceno(OBJMAN_MOUSE_OBJNO)!=GFX_NOSPRITE) {
            /* draw the cursor */
            gfx->putsprite(device_x,device_y,objman->getsourceno(OBJMAN_MOUSE_OBJNO),displaydata);
        }
    }
    /* draw this */
